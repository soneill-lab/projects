...
for (i in 1:10) {
  AvgWW_temp<- read.table(paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Average_WeaningWeight_F/yd.Landrace.AvgWW_full.CV",i,".variance.components.model.csv", sep=""), header = T, sep=",")
  AvgWW_temp_N<- read.table(paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Average_WeaningWeight_F/Landrace.AvgWW_N",i,".csv", sep=""), header = F)
...
AvgWW_temp_N<- AvgWW_temp_N[order(AvgWW_temp_N$V1),]
  ...
  # AA effect
  foi_temp_T<- foi[foi$V1 %in% AvgWW_temp$EGO]
  foi_temp_T<- foi_temp_T[order(foi_temp_T$V1),]
  foi_temp_T2<- select_if(foi_temp_T, colnames(foi_temp_T) %in% AvgWW_temp$EGO)
 ...
 # write.table(p, file=paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Average_WeaningWeight_F/AvgWW",i,"_p.csv", sep=""), quote = F, col.names = F, row.names = F)
  ...
  # adjust by F
  AvgWW_Adj<- read.table("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Average_WeaningWeight_F/yd.Landrace.AAvgWW_full.CV.Adj.variance.components.model.csv", header = T, sep=",")
  AvgWW_Adj<- as.data.frame(AvgWW_Adj)
  AvgWW_Adj<- AvgWW_Adj[,c("EGO","F_FIT","yd_S")]
  names(AvgWW_Adj)<- c("ID","F","S")
  y_temp<- merge(y_temp, AvgWW_Adj, by="ID")
  y_temp$AvgWW_Adj<- y_temp$yd - y_temp$F -y_temp$S

  
  y_temp<- merge(y_temp, p, by="ID")
  
  # write.table(y_temp, file=paste(/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Average_WeaningWeight_F/AvgWW",i,"_y.csv", sep=""), quote = F, col.names = F, row.names = F, sep=",")
  # 
  corr<- cor(y_temp$PEBV, y_temp$AvgWW_Adj, use="p")
  corr2<- cor(y_temp$AEBV, y_temp$AvgWW_Adj, use="p")
  print(corr)
  print(corr2)
...
write.table(Acc, file="/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Average_WeaningWeight_F/AvgWW_Acc_epi.csv", quote = F, col.names = F, row.names = F, sep=",")
