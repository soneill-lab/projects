...
foi<- fread("yorkshire.AA.grm.txt")
dom<- fread("yorkshire.dom.grm.txt")
ad<- fread("yorkshire.AD.grm.txt")
dd<- fread("yorkshire.DD.grm.txt")

Acc<- data.frame(matrix(NA, nrow = 10, ncol = 2))

for (i in 1:10) {
  ADG_temp<- read.table(paste("/home/varcomp/Yorkshire_Breed/Cross_Validation/ADG_Finisher_F/yd.Yorkshire_ADG_Finisher_full.CV",i,".variance.components.model.csv", sep=""), header = T, sep=",")
  ADG_temp_N<- read.table(paste("/home/varcomp/Yorkshire_Breed/Cross_Validation/ADG_Finisher_F/Yorkshire_ADG_Finisher_N",i,".csv", sep=""), header = F)
 ...
 # write.table(p, file=paste("/home/varcomp/Yorkshire_Breed/Cross_Validation/ADG_Finisher_F/ADG_Finisher",i,"_p.csv", sep=""), quote = F, col.names = F, row.names = F)
  ...
  # adjust by F
  ADG_Adj<- read.table("/home/varcomp/Yorkshire_Breed/Cross_Validation/ADG_Finisher_F/yd.Landrace.ADG_Finisher_full.CV.Adj.variance.components.model.csv", header = T, sep=",")
  ...
  # write.table(y_temp, file=paste(/home/varcomp/Yorkshire_Breed/Cross_Validation/ADG_Finisher_F/ADG_Finisher",i,"_y.csv", sep=""), quote = F, col.names = F, row.names = F, sep=",")
  # 
...
write.table(Acc, file="/home/varcomp/Yorkshire_Breed/Cross_Validation/ADG_F/ADG_Finisher_Acc_epi.csv", quote = F, col.names = F, row.names = F, sep=",")
