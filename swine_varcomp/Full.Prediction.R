...
for (i in 1:10) {
  NBA_temp<- read.table(paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/yd.Landrace_NBA_full.CV",i,".variance.components.model.csv", sep=""), header = T, sep=",")
  NBA_temp_N<- read.table(paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/Landrace_NBA_N",i,".csv", sep=""), header = F)
  ...
  # AA effect
  foi_temp_T<- foi[foi$V1 %in% NBA_temp$EGO]
  foi_temp_T<- foi_temp_T[order(foi_temp_T$V1),]
  foi_temp_T2<- select_if(foi_temp_T, colnames(foi_temp_T) %in% NBA_temp$EGO)
  ...
  f_t<- as.vector(NBA_temp$yd_AA)
  f<- F_nxt %*% solve(F_txt) %*%f_t
  
  # AD effect
  ad_temp_T<- ad[ad$V1 %in% NBA_temp$EGO]
  ad_temp_T<- ad_temp_T[order(ad_temp_T$V1),]
  ad_temp_T2<- select_if(ad_temp_T, colnames(ad_temp_T) %in% NBA_temp$EGO)
  ad_temp_T2<- ad_temp_T2 %>% select(sort(names(ad_temp_T2)))
  AD_txt<- as.matrix(ad_temp_T2)
  
  ad_temp_N<- ad[ad$V1 %in% NBA_temp_N]
  ad_temp_N<- ad_temp_N[order(ad_temp_N$V1),]
  ad_temp_N2<- select_if(ad_temp_N, colnames(ad_temp_N) %in% NBA_temp$EGO)
  ...
  # DD effect
  dd_temp_T<- dd[dd$V1 %in% NBA_temp$EGO]
  dd_temp_T<- dd_temp_T[order(dd_temp_T$V1),]
  dd_temp_T2<- select_if(dd_temp_T, colnames(dd_temp_T) %in% NBA_temp$EGO)
  dd_temp_T2<- dd_temp_T2 %>% select(sort(names(dd_temp_T2)))
  DD_txt<- as.matrix(dd_temp_T2)
  
  dd_temp_N<- dd[dd$V1 %in% NBA_temp_N]
  dd_temp_N<- dd_temp_N[order(dd_temp_N$V1),]
  dd_temp_N2<- select_if(dd_temp_N, colnames(dd_temp_N) %in% NBA_temp$EGO)
  dd_temp_N2<- dd_temp_N2 %>% select(sort(names(dd_temp_N2)))
  DD_nxt<- as.matrix(dd_temp_N2)
  
  
  dd_t<- as.vector(NBA_temp$yd_DD)
  dxd<- DD_nxt %*% solve(DD_txt) %*%dd_t
  
  # Dominance effect
  dom_temp_T<- dom[dom$V1 %in% NBA_temp$EGO]
  dom_temp_T<- dom_temp_T[order(dom_temp_T$V1),]
  dom_temp_T2<- select_if(dom_temp_T, colnames(dom_temp_T) %in% NBA_temp$EGO)
  dom_temp_T2<- dom_temp_T2 %>% select(sort(names(dom_temp_T2)))
  D_txt<- as.matrix(dom_temp_T2)
  
  dom_temp_N<- dom[dom$V1 %in% NBA_temp_N]
  dom_temp_N<- dom_temp_N[order(dom_temp_N$V1),]
  dom_temp_N2<- select_if(dom_temp_N, colnames(dom_temp_N) %in% NBA_temp$EGO)
...
write.table(Acc, file="/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/NBA_F/NBA_Acc_epi.csv", quote = F, col.names = F, row.names = F, sep=",")
