...
for (i in 1:10) {
 ...
 write.table(p, file=paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/BF",i,"_p.csv", sep=""), quote = F, col.names = F, row.names = F)
  
  # get phenotype
  y<- read.table("Landrace_Backfat_Filtered", header = T, sep = ",")
  names(y)<- c("ID", "yd")
 BF_temp_N<- as.data.frame(BF_temp_N)
  ...
  # adjust by F
  BF_Adj<- read.table("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/ADG_a/yd.Landrace.BF_add.CV.Adj.variance.components.model.csv", header = T, sep=",")
  BF_Adj<- as.data.frame(BF_Adj)
  BF_Adj<- BF_Adj[,c("EGO","F_FIT","yd_S")]
  names(BF_Adj)[1]<- "ID"
  y_temp<- merge(y_temp,BF_Adj, by="ID")
  y_temp$BF_Adj<- y_temp$yd - y_temp$F_FIT - y_temp$yd_S
  
  y_temp<- merge(y_temp, p, by="ID")
  write.table(y_temp, file=paste("/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/BF",i,"_y.csv", sep=""), quote = F, col.names = F, row.names = F, sep=",")
...
write.table(Acc, file="/home/varcomp/Landrace_Breed/New_Pipeline/New_Phenotypes/Cross_Validation/BF_Acc.csv", quote = F, col.names = F, row.names = F, sep=",")
